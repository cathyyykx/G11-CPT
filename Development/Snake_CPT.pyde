...
def start_game():
    global applex, appley, snakesize, time, angle, apple
    background(255)
    headx[1] = 200
    heady[1] = 200
...
def check_if_dead():
    for i in range(2, int(snakesize)): 
        if headx[1] == headx[i] and heady[1] == heady[i]: # eat itself
            ...
            text("Score: " + str(apple) + " units long", 200, 175) 
            text("To restart, press Ctrl.", 200, 200) 
            stop_game = True
        if headx[1] >= (width - 10) or heady[1] >= (height - 10) or headx[1] <= 0 or heady[1] <= 0:
            background(0)
            fill(0) 
            rect(125, 125, 160, 100) 
            fill(255) 
            ...
            text("Press Ctrl to RESTART", 200, 200) 
            stop_game = True
